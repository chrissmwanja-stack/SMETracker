...
fun PaymentBreakdownScreen(viewModel: SMEViewModel, navController: NavController) {
    ...
    val pdfData = remember(business, uiState.analytics, sales) {
        ReportPdfMappers.paymentBreakdownReport(business, uiState.analytics, sales)
    }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                actions = {
                    ReportPdfExportActions(pdfExport, pdfData, "payment_breakdown_report")
                }
            )
        }
    ) { padding ->
